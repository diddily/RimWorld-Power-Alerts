...
namespace Power_Alerts.Alerts
{
    class Alert_GeneratorFuelLow : Alert
    {
        ...
        {
            return Find.Maps.FirstOrDefault(m => m.IsPlayerHome).powerNetManager.AllNetsListForReading.Where(pn => pn.batteryComps.Count() > 0 || pn.powerComps.Any(pc => pc != null && pc.Props.basePowerConsumption > 0.0f)).SelectMany(pn => pn.powerComps.Where(pc => pc != null && pc.PowerOn && pc.Props.basePowerConsumption < 0.0f && IsFuelLow(pc.parent.TryGetComp<CompRefuelable>())));
        }
