...
namespace Power_Alerts.Alerts
{
    class Alert_DrainingBatteries : Alert
    {
        ...
        {
            return Find.Maps.FirstOrDefault(m => m.IsPlayerHome).powerNetManager.AllNetsListForReading.Where(pn => GetBatteryState(pn) != BatteryState.Normal).SelectMany(pn => pn.batteryComps).Select(bc => bc.parent as Building).Where(b => b.Faction.IsPlayer);
        }
