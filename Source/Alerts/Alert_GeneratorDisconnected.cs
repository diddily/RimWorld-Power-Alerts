...
namespace Power_Alerts.Alerts
{
    class Alert_GeneratorDisconnected : Alert
    {
        ...
        {
            return Find.Maps.FirstOrDefault(m => m.IsPlayerHome).powerNetManager.AllNetsListForReading.Where(pn => !pn.powerComps.Any(pc => pc.Props.basePowerConsumption > 0.0f)).SelectMany(pn => pn.powerComps.Where(pc => pc.PowerOutput > 0.0f)).Select(pc => pc.parent as Building);
        }
