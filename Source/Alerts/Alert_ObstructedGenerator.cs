...
namespace Power_Alerts.Alerts
{
    class Alert_ObstructedGenerator : Alert
    {
        ...
        {
            return Find.Maps.FirstOrDefault(m => m.IsPlayerHome).powerNetManager.AllNetsListForReading.Where(pn => pn.batteryComps.Count() > 0 || pn.powerComps.Any(pc => pc != null && pc.Props.PowerConsumption > 0.0f)).SelectMany(pn => pn.powerComps).Where(pct => IsObstructedGenerator(pct)).Select(pct => pct.parent as Building).Where(b => (b.Faction?.IsPlayer ?? false));
        }
